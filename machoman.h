...
__attribute__((always_inline)) struct mach_header *get_mach_header(macho_map_t *map);

__attribute__((always_inline)) struct load_command **find_all_load_commands(struct mach_header *mh);

__attribute__((always_inline)) struct load_command *find_load_command(struct mach_header *mh, uint32_t lc);
__attribute__((always_inline)) struct segment_command *find_segment_command(struct mach_header *mh, const char *segname);
